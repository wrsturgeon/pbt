...
impl<T: Clone + Refine> Refiner<T> {
    ...
    pub fn increment_size(&mut self) {
        match *self {
            ...
            Self::Cons {
                ref mut head_size, ..
            } => *head_size = Some(head_size.map_or(0, |head_size| head_size + 1)),
        }
    }
    ...
    pub fn new(slice: &[T], size: usize) -> Self {
        match *slice {
            ...
            [ref head, ref tail @ ..] => Self::Cons {
                ...
                head: None,
                tail: Box::new(Self::new(tail, tail.len())),
                original: head.clone(),
            },
        }
    }
    ...
    pub fn next_acc(&mut self, acc: &mut Vec<T>) -> Option<()> {
        match *self {
            Self::Nil { remaining_size } => (remaining_size == 0).then_some(()),
            Self::Cons {
                ref mut head_size,
                ref mut head,
                ref mut tail,
